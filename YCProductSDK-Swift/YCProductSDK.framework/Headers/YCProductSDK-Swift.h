...
/// App 启动测量数据的测量方式
typedef SWIFT_ENUM(uint8_t, YCAppControlHealthDataMeasureType, closed) {
...
/// App 启动测量结果
typedef SWIFT_ENUM(uint8_t, YCAppControlMeasureHealthDataResult, closed) {
...
/// App 启动测量数据类型
typedef SWIFT_ENUM(uint8_t, YCAppControlMeasureHealthDataType, closed) {
...
/// 测量模式
typedef SWIFT_ENUM(uint8_t, YCAppControlMeasureMode, closed) {
...
/// 二维码颜色
typedef SWIFT_ENUM(uint8_t, YCBodyTemperatureQRCodeColor, closed) {
...
/// 采集数据类型
typedef SWIFT_ENUM(uint8_t, YCCollectDataType, closed) {
...
/// 删除健康数据的类型定义
typedef SWIFT_ENUM(uint8_t, YCDeleteHealthDataType, closed) {
...
/// 日程的时间类型
typedef SWIFT_ENUM(uint8_t, YCDevcieScheduleEventType, closed) {
...
/// 闹钟操作类型
typedef SWIFT_ENUM(uint8_t, YCDeviceAlarmOperatoreType, closed) {
...
/// 闹钟类型
typedef SWIFT_ENUM(uint8_t, YCDeviceAlarmType, closed) {
...
/// 3.5.2.7/8 防丢提醒设置
typedef SWIFT_ENUM(uint8_t, YCDeviceAntiLostType, closed) {
...
/// 电池电量状态
typedef SWIFT_ENUM(uint8_t, YCDeviceBatterystate, closed) {
...
/// 3.5.2.23 血压范围设置(等级)
typedef SWIFT_ENUM(uint8_t, YCDeviceBloodPressureLevel, closed) {
...
/// 3.5.2.34 息屏时间
typedef SWIFT_ENUM(uint8_t, YCDeviceBreathScreenInterval, closed) {
...
/// 设备允许连接状态
typedef SWIFT_ENUM(uint8_t, YCDeviceControlAllowConnectionState, closed) {
  YCDeviceControlAllowConnectionStateAgree = 0,
  YCDeviceControlAllowConnectionStateRefuse = 1,
};

/// 设备控制的运动模式
typedef SWIFT_ENUM(uint8_t, YCDeviceControlDeviceSportState, closed) {
...
/// 设备控制拍照
typedef SWIFT_ENUM(uint8_t, YCDeviceControlPhotoState, closed) {
...
/// 设备控制的一般状态
typedef SWIFT_ENUM(uint8_t, YCDeviceControlState, closed) {
  YCDeviceControlStateStop = 0,
  YCDeviceControlStateStart = 1,
};

/// 设备控制的数据类型
typedef SWIFT_ENUM(uint8_t, YCDeviceControlType, closed) {
...
/// 3.5.2.28 数据采集配置
typedef SWIFT_ENUM(uint8_t, YCDeviceDataCollectionType, closed) {
...
/// 3.5.2.21 显示屏亮度设置
typedef SWIFT_ENUM(uint8_t, YCDeviceDisplayBrightnessLevel, closed) {
...
/// 3.5.2.5 单位设置
/// 距离单位
typedef SWIFT_ENUM(uint8_t, YCDeviceDistanceType, closed) {
  YCDeviceDistanceTypeKm = 0,
  YCDeviceDistanceTypeMile = 1,
};

/// 心电电极位置
typedef SWIFT_ENUM(uint8_t, YCDeviceElectrodePosition, closed) {
...
/// 3.5.2.34 事件间隔时间
typedef SWIFT_ENUM(uint8_t, YCDeviceEventInterval, closed) {
...
/// 运动状态类型
typedef SWIFT_ENUM(uint8_t, YCDeviceExerciseHeartRateType, closed) {
...
/// 3.5.2.4 用户信息设置
typedef SWIFT_ENUM(uint8_t, YCDeviceGender, closed) {
...
/// 3.5.2.11 通知提醒类型
typedef SWIFT_ENUM(NSUInteger, YCDeviceInfoPushType, closed) {
...
/// 3.5.2.19 语言设置
typedef SWIFT_ENUM(uint8_t, YCDeviceLanguageType, closed) {
...
/// MCU
typedef SWIFT_ENUM(uint8_t, YCDeviceMCUType, closed) {
...
/// 马达震动类型
typedef SWIFT_ENUM(uint8_t, YCDeviceMotorVibrationType, closed) {
  YCDeviceMotorVibrationTypeAlarm = 0,
};

/// 设备实时状态
typedef SWIFT_ENUM(uint8_t, YCDeviceRealState, closed) {
...
/// 提醒设置
typedef SWIFT_ENUM(uint8_t, YCDeviceRemindType, closed) {
  YCDeviceRemindTypeDeviceDisconnected = 0,
  YCDeviceRemindTypeSportsCompliance = 1,
};

/// 提醒设置状态
typedef SWIFT_ENUM(uint8_t, YCDeviceReminderSettingState, closed) {
...
/// 数据存储
typedef SWIFT_ENUM(uint8_t, YCDeviceSenserSaveDataType, closed) {
...
/// 3.5.2.22 肤色设置
typedef SWIFT_ENUM(uint8_t, YCDeviceSkinColorLevel, closed) {
...
/// 运动开关
typedef SWIFT_ENUM(uint8_t, YCDeviceSportState, closed) {
...
/// 运动类型定义
typedef SWIFT_ENUM(uint8_t, YCDeviceSportType, closed) {
...
/// 设备模式
typedef SWIFT_ENUM(uint8_t, YCDeviceSystemOperator, closed) {
...
/// 温度单位
typedef SWIFT_ENUM(uint8_t, YCDeviceTemperatureType, closed) {
...
/// 时间格式
typedef SWIFT_ENUM(uint8_t, YCDeviceTimeType, closed) {
...
/// 3.5.2.6 时间重复格式
typedef SWIFT_ENUM(uint8_t, YCDeviceWeekRepeat, closed) {
...
/// 体重单位
typedef SWIFT_ENUM(uint8_t, YCDeviceWeightType, closed) {
  YCDeviceWeightTypeKg = 0,
  YCDeviceWeightTypeLb = 1,
};

/// 工作模式
typedef SWIFT_ENUM(uint8_t, YCDeviceWorkModeType, closed) {
...
/// ECG结果
typedef SWIFT_ENUM(NSUInteger, YCECGResultType, closed) {
/// 测量失败
...
/// 嵌入式外设固件类型
typedef SWIFT_ENUM(uint8_t, YCEmbeddedPeripheralFirmwareType, closed) {
...
/// 测量模式
typedef SWIFT_ENUM(uint8_t, YCHealthDataBloodPressureMode, closed) {
...
/// 健康数据测量模式
typedef SWIFT_ENUM(uint8_t, YCHealthDataMeasureMode, closed) {
...
/// 深睡和浅睡标记
typedef SWIFT_ENUM(NSInteger, YCHealthDataSleepType, closed) {
...
/// 运动启动方式
typedef SWIFT_ENUM(uint8_t, YCHealthDataSportModeStartMethod, closed) {
...
/// 佩戴脱落状态
typedef SWIFT_ENUM(uint8_t, YCHealthDataWearState, closed) {
...
/// 预警信息
typedef SWIFT_ENUM(uint8_t, YCHealthParametersState, closed) {
...
/// 健康状态
typedef SWIFT_ENUM(uint8_t, YCHealthState, closed) {
...
/// 健康数据回写类型
typedef SWIFT_ENUM(uint8_t, YCMeasurementDataWritebackType, closed) {
...
/// 个人信息类型
typedef SWIFT_ENUM(uint8_t, YCPersonalInfoType, closed) {
...
/// \param completion 设置结果
///
+ (void)setDeviceTemperatureAlarm:(CBPeripheral * _Nullable)peripheral isEnable:(BOOL)isEnable highTemperatureIntegerValue:(uint8_t)highTemperatureIntegerValue highTemperatureDecimalValue:(uint8_t)highTemperatureDecimalValue lowTemperatureIntegerValue:(int8_t)lowTemperatureIntegerValue lowTemperatureDecimalValue:(uint8_t)lowTemperatureDecimalValue completion:(void (^ _Nullable)(enum YCProductState, id _Nullable))completion;
/// 温度监测
/// \param peripheral 连接设备
///
/// \param isEnable 是否开启
...
/// 日志等级 (目前只支持两个，开和关)
typedef SWIFT_ENUM(NSInteger, YCProductLogLevel, closed) {
  YCProductLogLevelOff = 0,
  YCProductLogLevelNormal = 1,
};

/// 操作状态标示
typedef SWIFT_ENUM(NSInteger, YCProductState, closed) {
...
/// 查询健康数据的类型定义
typedef SWIFT_ENUM(uint8_t, YCQueryHealthDataType, closed) {
...
/// 查询采样率
typedef SWIFT_ENUM(uint8_t, YCQuerySampleRateType, closed) {
...
/// 实时数据上传
typedef SWIFT_ENUM(uint8_t, YCRealTimeDataType, closed) {
...
/// 接收到实时数据
typedef SWIFT_ENUM(uint8_t, YCReceivedRealTimeDataType, closed) {
...
/// 3.5.2.3 目标设置类型
typedef SWIFT_ENUM(uint8_t, YCSettingGoalType, closed) {
...
/// 3.5.2.30 温度模式设置
typedef SWIFT_ENUM(uint8_t, YCSettingTemperatureModeType, closed) {
  YCSettingTemperatureModeTypeBodyTemperature = 0,
};

/// 通讯录同步状态
typedef SWIFT_ENUM(uint8_t, YCSyncAddressBookState, closed) {
...
/// 预警信息类型
typedef SWIFT_ENUM(uint8_t, YCWarningInformationType, closed) {
...
/// 波形类型选择
typedef SWIFT_ENUM(uint8_t, YCWaveDataType, closed) {
  YCWaveDataTypePpg = 0,
  YCWaveDataTypeEcg = 1,
...
/// 波形上传控制
typedef SWIFT_ENUM(uint8_t, YCWaveUploadState, closed) {
...
/// 天气编码类型
typedef SWIFT_ENUM(uint8_t, YCWeatherCodeType, closed) {
...
/// 天气中的月相信息
typedef SWIFT_ENUM(uint8_t, YCWeatherMoonType, closed) {
...
/// 3.5.2.1 星期表示
typedef SWIFT_ENUM(uint8_t, YCWeekDay, closed) {
  YCWeekDayMonday = 0,
  YCWeekDayTuesday = 1,
