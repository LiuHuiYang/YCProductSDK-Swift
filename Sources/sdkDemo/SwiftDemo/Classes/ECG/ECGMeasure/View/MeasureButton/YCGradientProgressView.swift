...
open class YCGradientProgressView: UIView {
    
    //进度条完成部分的渐变颜色，设置单个为纯色，设置多个为渐变色
    public internal(set) var progressColors: [UIColor] = [.systemBlue] {
        didSet {
    ...
    //进度条完成部分的圆角半径
    public internal(set) var progressCornerRadius: CGFloat = 0 {
        didSet {
    ...
    //进度完成部分的内间距
    public internal(set) var progressEdgeInsets: UIEdgeInsets = .zero {
        didSet {
            setNeedsLayout()
        }
    }
    ...
    //动画持续时间
    public internal(set) var animationDuration: TimeInterval = 0.3
    
    //动画时间函数
    public internal(set) var timingFunction: CAMediaTimingFunction = CAMediaTimingFunction(name: .default)
    
    //进度更新动画过程中的回调，在这里可以拿到当前进度及进度条的frame
    public internal(set) var progressUpdating: ((Float, CGRect) -> ())?
